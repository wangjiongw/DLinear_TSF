...
class Dataset_eload(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 target='OT', scale=True, timeenc=0, freq='h'):
        """initialize Electricity load dataset

        :param str root_path: path of root directory
        :param str flag: type of dataset, defaults to 'train'
        :param list size: size of each frame data, defaults to None
        :param str features: type of input feature, defaults to 'S'
        :param str data_path: path of dataset file, defaults to 'dg_load.csv'
        :param str target: name of target feature, defaults to 'OT'
        :param bool scale: scale data or not, defaults to True
        :param int timeenc: type of time encoding, defaults to 0
        :param str freq: sample frequency of data, defaults to 'h'
        """
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            ...
            self.seq_len = size[0]          # input sequence length (lookback window)
            self.label_len = size[1]        # length of token data
    ...
    def __read_data__(self):
        ...
        '''
        df_raw.columns: ['date', ...(other features), target feature]
        df_raw.columns: ['date', DGS, ...(other district)]
        '''
        cols = list(df_raw.columns)
        cols.remove(self.target)
        cols.remove('date')
        ...
        if self.features == 'M' or self.features == 'MS':
            cols_data = df_raw.columns[1:]      # multivariate
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_data = df_raw[[self.target]]         # OT -> OT

        if self.scale:      # scale train set
            train_data = df_data[border1s[0]: border2s[0]]
            self.scaler.fit(train_data.values)
            # print(self.scaler.mean_)
            # exit()
